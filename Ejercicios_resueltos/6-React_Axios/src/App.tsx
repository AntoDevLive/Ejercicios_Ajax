...
function App() {
  ...
  async function loadAll(): Promise<void> {
    try {
      const res = await axios.get<IUsuario[]>('http://localhost/ejercicios_ajax/index.php');
      setUsuario(res.data);
      console.log(usuario);
  ...
  async function createUser(): Promise<void> {
    ...
    const nuevoUsuario: IUsuario = {
      nombre: 'reactPOST',
      apellido: 'reactPOST',
      email: 'reactPOST@correo',
      telefono: 654321987
    }

    try {
      const res = await axios.post('http://localhost/ejercicios_ajax/index.php', nuevoUsuario);
      console.log(res);
    } catch (error) {
      console.log(error)
    }
